...
def agent_wakeup(neuralcloud):
  ...
  if os.path.isfile(neuralcloud):
    print("[DEBUG]: Pre-existing neural cloud found. Loading...")
    loaded_table = load_table(neuralcloud)
    ai_name = loaded_table.where("Speaker", "Name").column("Contents").item(0)
    print(f"[DEBUG]: Neural cloud for {ai_name} loaded.")
    return loaded_table
  else:
    print("[DEBUG]: No existing neural cloud found. Creating...")
...
def agent_load_memory(embed_table, user_input, n, k, embed_model):
